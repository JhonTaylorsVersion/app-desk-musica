...
const handleDownloadLyricsWrapper = (
  spotifyId: string,
  name: string,
  artists: string,
  albumName?: string,
...
const handleDownloadCoverWrapper = (
  coverUrl: string,
  trackName: string,
  artistName: string,
  albumName?: string,
...
const toggleSelectAll = (tracks?: any[]) => {
  ...
  const allSelected = trackIds.every((id: string) =>
    selectedTracks.value.includes(id),
  );
  if (allSelected) {
    ...
    selectedTracks.value = Array.from(
      new Set([...selectedTracks.value, ...trackIds]),
    );
  }
};
...
const handleDownloadBatch = () => {
  ...
  const isAlbumView = currentView.value === "album";
  const playlistSyncContext =
    currentView.value === "playlist" &&
    "playlist_info" in metadata.value &&
    url.value.includes("/playlist/")
      ? {
          kind: "playlist" as const,
          spotifyUrl: url.value,
          playlistName: metadata.value.playlist_info.name,
        }
      : undefined;
  void downloadSelected(
    selectedTracks.value,
    metadata.value.track_list,
    getFolderNameForMetadata(metadata.value),
    isAlbumView,
    playlistSyncContext,
  );
};
...
const handleDownloadSingleTrack = (...args: any[]) => {
  if (args.length > 0) {
    ...
    void downloadTrack({
      ...
      is_album_context: currentView.value === "album",
  ...
  <div class="space-y-6">
    ...
    <div v-if="!searchMode">
      ...
      <template v-else>
        ...
        <SfAlbumView
          ...
          :downloading-track="downloadingTrack"
          :bulk-download-type="bulkDownloadType"
          ...
          @stop-download="handleStopDownload"
          @download-track="handleDownloadSingleTrack"
          @download-all-lyrics="
            () =>
              apiDownloadAllLyrics(
                metadata.track_list,
          ...
          @artist-click="
            (artist: { id: string; name: string; external_urls: string }) =>
              void handleArtistClick(artist)
          "
          @track-click="
            (track: { external_urls?: string }) =>
              track.external_urls && handleFetchUrl(track.external_urls)
          "
          @back="handleBack"
        />
    ...
    <Dialog :open="showAlbumDialog" @update:open="setShowAlbumDialog">
      <DialogContent class="sm:max-w-[425px]">
        ...
        <div v-if="selectedAlbum" class="py-2">
          <p class="font-medium bg-muted/50 rounded-md px-3 py-2">
            {{ selectedAlbum.name }}
          </p>
        </div>
        <DialogFooter>
    ...
    <Dialog :open="showVpnAdviceDialog" @update:open="setShowVpnAdviceDialog">
      <DialogContent class="max-w-md">
        ...
        <DialogFooter>
          <Button @click="setShowVpnAdviceDialog(false)"> Close </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
