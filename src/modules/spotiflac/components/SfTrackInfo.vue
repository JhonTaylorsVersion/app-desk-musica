...
import type {
  TrackMetadata,
  TrackAvailability,
} from "@/modules/spotiflac/types/api";
...
const handleDownload = () => {
  emit(
    ...
    props.track.album_artist,
    props.track.release_date,
    props.track.images,
    props.track.track_number,
    props.track.disc_number,
    props.track.total_tracks,
    props.track.total_discs,
    props.track.copyright,
    props.track.publisher,
  );
};
    ...
    <CardContent class="px-6 py-6">
      <div class="flex gap-6 items-start">
        <div class="shrink-0">
          <div
            v-if="track.images"
            class="relative w-48 h-48 rounded-md shadow-lg overflow-hidden"
          >
            <img
              :src="track.images"
              :alt="track.name"
            ...
            />
            <div
              class="absolute bottom-1 right-1 bg-black/80 text-white px-1.5 py-0.5 text-xs font-medium rounded"
            >
        ...
        <div class="flex-1 space-y-4 min-w-0">
          <div class="space-y-1">
            <div class="flex items-center gap-3">
              ...
              <span
                ...
                class="inline-flex items-center justify-center bg-red-600 text-white text-[10px] h-4 w-4 rounded shrink-0"
                title="Explicit"
                >E</span
              >
            ...
            <p class="text-lg text-muted-foreground">
              <template v-if="clickableArtists.length > 0">
                <span
                  v-for="(artist, index) in clickableArtists"
          ...
          <div class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            <div class="space-y-3">
              <div>
                <p
                  class="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/60 mb-0.5"
                >
                  Album
                </p>
                <p class="font-medium truncate leading-tight">
                  <span
                    v-if="hasAlbumClick"
                    class="cursor-pointer hover:underline text-primary/90"
                    @click="
                      emit('albumClick', {
                        id: track.album_id!,
            ...
            <div class="space-y-3">
              ...
              <div v-if="track.copyright">
                ...
                <p
                  class="text-[11px] text-muted-foreground leading-snug line-clamp-2"
          ...
          <TooltipProvider v-if="track.spotify_id">
            <div class="flex gap-2 flex-wrap">
              <Button
                ...
                :disabled="
                  isDownloading || downloadingTrack === track.spotify_id
                "
              >
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <Pause
                      v-else-if="playingTrack === track.spotify_id"
                      class="h-4 w-4"
                    />
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  <Button
                    @click="handleDownloadLyrics"
                    variant="outline"
                    size="icon"
                    :disabled="downloadingLyricsTrack === track.spotify_id"
                  >
                    <Spinner
                      v-if="downloadingLyricsTrack === track.spotify_id"
                    />
                    <FileCheck
                      v-else-if="skippedLyrics"
                      class="h-4 w-4 text-yellow-500"
                    />
                    <CheckCircle
                      v-else-if="downloadedLyrics"
                      class="h-4 w-4 text-green-500"
                    />
                    <XCircle
                      v-else-if="failedLyrics"
              ...
              <Tooltip v-if="track.images">
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <FileCheck
                      v-else-if="skippedCover"
                      class="h-4 w-4 text-yellow-500"
                    />
                    <CheckCircle
                      v-else-if="downloadedCover"
                      class="h-4 w-4 text-green-500"
                    />
                    <XCircle
                      v-else-if="failedCover"
                      class="h-4 w-4 text-red-500"
                    />
                ...
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <template v-else-if="availability">
                      <CheckCircle
                        v-if="hasAvailabilityLinks(availability)"
                        class="h-4 w-4 text-green-500"
                      />
