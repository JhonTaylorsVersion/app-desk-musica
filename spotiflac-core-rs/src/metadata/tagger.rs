use crate::models::{AppConfig, TrackMetadata};
...
impl Tagger {
    ...
    pub fn embed_metadata(
        ...
        let display_artist = final_artist
        ...
        if let Some(upc) = &metadata.upc {
            ...
            tag.insert_text(
                ItemKey::Unknown(PREFERRED_UPC_TAG_KEY.to_string()),
                upc.clone(),
            );
        }
        ...
        if let Some(lyr) = lyrics {
            ...
            if ext == "m4a" || ext == "mp4" {
                let _ = crate::utils::ffmpeg::FFmpeg::embed_lyrics_to_m4a(
        ...
        tag.insert_text(
            ...
            "SpotiFLAC-rs (Rust Engine)".to_string(),
        );
