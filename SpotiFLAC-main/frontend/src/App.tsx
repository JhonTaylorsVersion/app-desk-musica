...
function App() {
    ...
    const [combinedDownloadingTrack, setCombinedDownloadingTrack] = useState<string | null>(null);
    const [combinedBulkDownloadType, setCombinedBulkDownloadType] = useState<"all" | "selected" | null>(null);
    ...
    const handleDownloadTrackWithLyrics = useCallback(async (track: TrackMetadata, options?: {
        folderName?: string;
        playlistName?: string;
        position?: number;
        isAlbum?: boolean;
        ...
        setCombinedDownloadingTrack(track.spotify_id);
        try {
            await download.handleDownloadTrack(track.spotify_id, track.name, track.artists, track.album_name, track.spotify_id, options?.folderName, track.duration_ms, options?.position ?? track.track_number, track.album_artist, track.release_date, track.images, track.track_number, track.disc_number, track.total_tracks, track.total_discs, track.copyright, track.publisher);
            await lyrics.handleDownloadLyrics(track.spotify_id, track.name, track.artists, track.album_name, options?.playlistName, options?.position ?? track.track_number, track.album_artist, track.release_date, track.disc_number, options?.isAlbum);
        }
        finally {
            setCombinedDownloadingTrack(null);
        }
    }, [download, lyrics]);
    const handleDownloadTracksWithLyrics = useCallback(async (tracks: TrackMetadata[], options: {
        folderName?: string;
        playlistName?: string;
        isAlbum?: boolean;
        mode: "all" | "selected";
        ...
        if (tracksWithId.length === 0) {
            toast.error("No tracks available for combined download");
            return;
        }
        setCombinedBulkDownloadType(options.mode);
        try {
            if (options.mode === "selected") {
                await download.handleDownloadSelected(tracksWithId.map((track) => track.spotify_id!), tracksWithId, options.folderName, options.isAlbum);
            }
            else {
                await download.handleDownloadAll(tracksWithId, options.folderName, options.isAlbum);
            }
            await lyrics.handleDownloadAllLyrics(tracksWithId, options.playlistName, undefined, options.isAlbum);
        }
        finally {
            setCombinedBulkDownloadType(null);
        }
    }, [download, lyrics]);
    const renderMetadata = () => {
        ...
        if ("track" in metadata.metadata) {
            ...
            return (<TrackInfo track={track} isDownloading={download.isDownloading || combinedDownloadingTrack === track.spotify_id} downloadingTrack={download.downloadingTrack} combinedDownloadingTrack={combinedDownloadingTrack} isDownloaded={download.downloadedTracks.has(trackId)} isFailed={download.failedTracks.has(trackId)} isSkipped={download.skippedTracks.has(trackId)} isQueued={download.queuedTracks.has(trackId)} downloadingLyricsTrack={lyrics.downloadingLyricsTrack} downloadedLyrics={lyrics.downloadedLyrics.has(track.spotify_id || "")} failedLyrics={lyrics.failedLyrics.has(track.spotify_id || "")} skippedLyrics={lyrics.skippedLyrics.has(track.spotify_id || "")} checkingAvailability={availability.checkingTrackId === track.spotify_id} availability={availability.availabilityMap.get(track.spotify_id || "")} downloadingCover={cover.downloadingCoverTrack === (track.spotify_id || `${track.name}-${track.artists}`)} downloadedCover={cover.downloadedCovers.has(track.spotify_id || `${track.name}-${track.artists}`)} failedCover={cover.failedCovers.has(track.spotify_id || `${track.name}-${track.artists}`)} skippedCover={cover.skippedCovers.has(track.spotify_id || `${track.name}-${track.artists}`)} onDownload={download.handleDownloadTrack} onDownloadWithLyrics={() => handleDownloadTrackWithLyrics(track, { isAlbum: true })} onDownloadLyrics={(spotifyId, name, artists, albumName, albumArtist, releaseDate, discNumber) => lyrics.handleDownloadLyrics(spotifyId, name, artists, albumName, undefined, undefined, albumArtist, releaseDate, discNumber, true)} onDownloadCover={(coverUrl, trackName, artistName, albumName, _playlistName, _position, trackId, albumArtist, releaseDate, discNumber) => cover.handleDownloadCover(coverUrl, trackName, artistName, albumName, undefined, undefined, trackId, albumArtist, releaseDate, discNumber)} onCheckAvailability={availability.checkAvailability} onOpenFolder={handleOpenFolder} onBack={metadata.resetMetadata}/>);
        }
        if ("album_info" in metadata.metadata) {
            const { album_info, track_list } = metadata.metadata;
            return (<AlbumInfo albumInfo={album_info} trackList={track_list} searchQuery={searchQuery} sortBy={sortBy} selectedTracks={selectedTracks} downloadedTracks={download.downloadedTracks} failedTracks={download.failedTracks} skippedTracks={download.skippedTracks} queuedTracks={download.queuedTracks} downloadingTrack={download.downloadingTrack} combinedDownloadingTrack={combinedDownloadingTrack} isDownloading={download.isDownloading || combinedBulkDownloadType !== null} bulkDownloadType={combinedBulkDownloadType ?? download.bulkDownloadType} downloadProgress={download.downloadProgress} currentDownloadInfo={download.currentDownloadInfo} currentPage={currentListPage} itemsPerPage={ITEMS_PER_PAGE} downloadedLyrics={lyrics.downloadedLyrics} failedLyrics={lyrics.failedLyrics} skippedLyrics={lyrics.skippedLyrics} downloadingLyricsTrack={lyrics.downloadingLyricsTrack} checkingAvailabilityTrack={availability.checkingTrackId} availabilityMap={availability.availabilityMap} downloadedCovers={cover.downloadedCovers} failedCovers={cover.failedCovers} skippedCovers={cover.skippedCovers} downloadingCoverTrack={cover.downloadingCoverTrack} isBulkDownloadingCovers={cover.isBulkDownloadingCovers} isBulkDownloadingLyrics={lyrics.isBulkDownloadingLyrics || combinedBulkDownloadType !== null} onSearchChange={handleSearchChange} onSortChange={setSortBy} onToggleTrack={toggleTrackSelection} onToggleSelectAll={toggleSelectAll} onDownloadTrack={download.handleDownloadTrack} onDownloadTrackWithLyrics={(track, position) => handleDownloadTrackWithLyrics(track, { folderName: album_info.name, playlistName: album_info.name, position, isAlbum: true })} onDownloadLyrics={(spotifyId, name, artists, albumName, _folderName, _isArtistDiscography, position, albumArtist, releaseDate, discNumber) => lyrics.handleDownloadLyrics(spotifyId, name, artists, albumName, album_info.name, position, albumArtist, releaseDate, discNumber, true)} onDownloadCover={(coverUrl, trackName, artistName, albumName, _folderName, _isArtistDiscography, position, trackId, albumArtist, releaseDate, discNumber) => cover.handleDownloadCover(coverUrl, trackName, artistName, albumName, album_info.name, position, trackId, albumArtist, releaseDate, discNumber, true)} onCheckAvailability={availability.checkAvailability} onDownloadAllLyrics={() => lyrics.handleDownloadAllLyrics(track_list, album_info.name, undefined, true)} onDownloadAllCovers={() => cover.handleDownloadAllCovers(track_list, album_info.name, true)} onDownloadAll={() => download.handleDownloadAll(track_list, album_info.name, true)} onDownloadAllWithLyrics={() => handleDownloadTracksWithLyrics(track_list, { mode: "all", folderName: album_info.name, playlistName: album_info.name, isAlbum: true })} onDownloadSelected={() => download.handleDownloadSelected(selectedTracks, track_list, album_info.name, true)} onDownloadSelectedWithLyrics={() => handleDownloadTracksWithLyrics(track_list.filter((track) => track.spotify_id && selectedTracks.includes(track.spotify_id)), { mode: "selected", folderName: album_info.name, playlistName: album_info.name, isAlbum: true })} onStopDownload={download.handleStopDownload} onOpenFolder={handleOpenFolder} onPageChange={setCurrentListPage} onBack={metadata.resetMetadata} onArtistClick={async (artist) => {
        ...
        if ("playlist_info" in metadata.metadata) {
            ...
            const playlistName = playlist_info.name || playlist_info.owner.name;
            return (<PlaylistInfo playlistInfo={playlist_info} trackList={track_list} searchQuery={searchQuery} sortBy={sortBy} selectedTracks={selectedTracks} downloadedTracks={download.downloadedTracks} failedTracks={download.failedTracks} skippedTracks={download.skippedTracks} queuedTracks={download.queuedTracks} downloadingTrack={download.downloadingTrack} combinedDownloadingTrack={combinedDownloadingTrack} isDownloading={download.isDownloading || combinedBulkDownloadType !== null} bulkDownloadType={combinedBulkDownloadType ?? download.bulkDownloadType} downloadProgress={download.downloadProgress} currentDownloadInfo={download.currentDownloadInfo} currentPage={currentListPage} itemsPerPage={ITEMS_PER_PAGE} downloadedLyrics={lyrics.downloadedLyrics} failedLyrics={lyrics.failedLyrics} skippedLyrics={lyrics.skippedLyrics} downloadingLyricsTrack={lyrics.downloadingLyricsTrack} checkingAvailabilityTrack={availability.checkingTrackId} availabilityMap={availability.availabilityMap} downloadedCovers={cover.downloadedCovers} failedCovers={cover.failedCovers} skippedCovers={cover.skippedCovers} downloadingCoverTrack={cover.downloadingCoverTrack} isBulkDownloadingCovers={cover.isBulkDownloadingCovers} isBulkDownloadingLyrics={lyrics.isBulkDownloadingLyrics || combinedBulkDownloadType !== null} onSearchChange={handleSearchChange} onSortChange={setSortBy} onToggleTrack={toggleTrackSelection} onToggleSelectAll={toggleSelectAll} onDownloadTrack={download.handleDownloadTrack} onDownloadTrackWithLyrics={(track, position) => handleDownloadTrackWithLyrics(track, { folderName: playlistName, playlistName, position })} onDownloadLyrics={(spotifyId, name, artists, albumName, _folderName, _isArtistDiscography, position, albumArtist, releaseDate, discNumber) => lyrics.handleDownloadLyrics(spotifyId, name, artists, albumName, playlistName, position, albumArtist, releaseDate, discNumber)} onDownloadCover={(coverUrl, trackName, artistName, albumName, _folderName, _isArtistDiscography, position, trackId, albumArtist, releaseDate, discNumber) => cover.handleDownloadCover(coverUrl, trackName, artistName, albumName, playlistName, position, trackId, albumArtist, releaseDate, discNumber)} onCheckAvailability={availability.checkAvailability} onDownloadAllLyrics={() => lyrics.handleDownloadAllLyrics(track_list, playlistName)} onDownloadAllCovers={() => cover.handleDownloadAllCovers(track_list, playlistName)} onDownloadAll={() => download.handleDownloadAll(track_list, playlistName)} onDownloadAllWithLyrics={() => handleDownloadTracksWithLyrics(track_list, { mode: "all", folderName: playlistName, playlistName })} onDownloadSelected={() => download.handleDownloadSelected(selectedTracks, track_list, playlistName)} onDownloadSelectedWithLyrics={() => handleDownloadTracksWithLyrics(track_list.filter((track) => track.spotify_id && selectedTracks.includes(track.spotify_id)), { mode: "selected", folderName: playlistName, playlistName })} onStopDownload={download.handleStopDownload} onOpenFolder={handleOpenFolder} onPageChange={setCurrentListPage} onBack={metadata.resetMetadata} onAlbumClick={metadata.handleAlbumClick} onArtistClick={async (artist) => {
                    const pendingArtistUrl = artist.external_urls.replace(/\/$/, "") + "/discography/all";
                    setSpotifyUrl(pendingArtistUrl);
                    const artistUrl = await metadata.handleArtistClick(artist);
                    if (artistUrl) {
        ...
        if ("artist_info" in metadata.metadata) {
            ...
            return (<ArtistInfo artistInfo={artist_info} albumList={album_list} trackList={track_list} searchQuery={searchQuery} sortBy={sortBy} selectedTracks={selectedTracks} downloadedTracks={download.downloadedTracks} failedTracks={download.failedTracks} skippedTracks={download.skippedTracks} queuedTracks={download.queuedTracks} downloadingTrack={download.downloadingTrack} combinedDownloadingTrack={combinedDownloadingTrack} isDownloading={download.isDownloading || combinedBulkDownloadType !== null} bulkDownloadType={combinedBulkDownloadType ?? download.bulkDownloadType} downloadProgress={download.downloadProgress} currentDownloadInfo={download.currentDownloadInfo} currentPage={currentListPage} itemsPerPage={ITEMS_PER_PAGE} downloadedLyrics={lyrics.downloadedLyrics} failedLyrics={lyrics.failedLyrics} skippedLyrics={lyrics.skippedLyrics} downloadingLyricsTrack={lyrics.downloadingLyricsTrack} checkingAvailabilityTrack={availability.checkingTrackId} availabilityMap={availability.availabilityMap} downloadedCovers={cover.downloadedCovers} failedCovers={cover.failedCovers} skippedCovers={cover.skippedCovers} downloadingCoverTrack={cover.downloadingCoverTrack} isBulkDownloadingCovers={cover.isBulkDownloadingCovers} isBulkDownloadingLyrics={lyrics.isBulkDownloadingLyrics || combinedBulkDownloadType !== null} onSearchChange={handleSearchChange} onSortChange={setSortBy} onToggleTrack={toggleTrackSelection} onToggleSelectAll={toggleSelectAll} onDownloadTrack={download.handleDownloadTrack} onDownloadTrackWithLyrics={(track, position) => handleDownloadTrackWithLyrics(track, { folderName: artist_info.name, playlistName: artist_info.name, position })} onDownloadLyrics={(spotifyId, name, artists, albumName, _folderName, _isArtistDiscography, position, albumArtist, releaseDate, discNumber) => lyrics.handleDownloadLyrics(spotifyId, name, artists, albumName, artist_info.name, position, albumArtist, releaseDate, discNumber)} onDownloadCover={(coverUrl, trackName, artistName, albumName, _folderName, _isArtistDiscography, position, trackId, albumArtist, releaseDate, discNumber) => cover.handleDownloadCover(coverUrl, trackName, artistName, albumName, artist_info.name, position, trackId, albumArtist, releaseDate, discNumber)} onCheckAvailability={availability.checkAvailability} onDownloadAllLyrics={() => lyrics.handleDownloadAllLyrics(track_list, artist_info.name)} onDownloadAllCovers={() => cover.handleDownloadAllCovers(track_list, artist_info.name)} onDownloadAll={() => download.handleDownloadAll(track_list, artist_info.name)} onDownloadAllWithLyrics={() => handleDownloadTracksWithLyrics(track_list, { mode: "all", folderName: artist_info.name, playlistName: artist_info.name })} onDownloadSelected={() => download.handleDownloadSelected(selectedTracks, track_list, artist_info.name)} onDownloadSelectedWithLyrics={() => handleDownloadTracksWithLyrics(track_list.filter((track) => track.spotify_id && selectedTracks.includes(track.spotify_id)), { mode: "selected", folderName: artist_info.name, playlistName: artist_info.name })} onStopDownload={download.handleStopDownload} onOpenFolder={handleOpenFolder} onAlbumClick={metadata.handleAlbumClick} onBack={metadata.resetMetadata} onArtistClick={async (artist) => {
                    const pendingArtistUrl = artist.external_urls.replace(/\/$/, "") + "/discography/all";
                    setSpotifyUrl(pendingArtistUrl);
                    const artistUrl = await metadata.handleArtistClick(artist);
                    if (artistUrl) {
