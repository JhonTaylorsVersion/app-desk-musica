...
function buildExpectedFilenameLikeBackend(
  trackName: string,
  artistName: string,
  albumName: string,
  albumArtist: string,
...
function buildInternalDuplicateFilenameTemplate(
  baseFilenameTemplate: string,
...
function buildBatchDuplicateFilenameTemplate(
  baseFilenameTemplate: string,
  playlistPosition: number,
): string {
  return buildInternalDuplicateFilenameTemplate(
    baseFilenameTemplate,
    `p${String(playlistPosition).padStart(4, "0")}`,
  );
}
function buildSingleTrackDuplicateFilenameTemplate(
  baseFilenameTemplate: string,
  spotifyId?: string,
  fallbackId?: string,
): string {
...
async function computeSha256HexFromBuffer(
  buffer: BufferSource,
): Promise<string> {
...
async function detectExistingTrackDifferences(
  ...
  const metadata = (await ReadAudioMetadata(
    existingFilePath,
  )) as ExistingTrackMetadata;
  const differences: string[] = [];
  const checks: Array<[string, string, string]> = [
    [
      "titulo",
      normalizeComparableText(metadata.title),
      normalizeComparableText(track.trackName),
    ],
    [
      "artista",
      normalizeComparableText(metadata.artist),
      normalizeComparableText(track.artistName),
    ],
    [
      "album",
      normalizeComparableText(metadata.album),
      normalizeComparableText(track.albumName),
    ],
    [
      "album artist",
      normalizeComparableText(metadata.album_artist),
      normalizeComparableText(track.albumArtist),
    ],
    [
      "fecha",
      normalizeComparableDate(metadata.release_date || metadata.year),
      normalizeComparableDate(track.releaseDate),
    ],
    [
      "track",
      normalizeComparableNumber(metadata.track_number),
      normalizeComparableNumber(track.spotifyTrackNumber),
    ],
    [
      "disc",
      normalizeComparableNumber(metadata.disc_number),
      normalizeComparableNumber(track.spotifyDiscNumber),
    ],
    [
      "total tracks",
      normalizeComparableNumber(metadata.track_total),
      normalizeComparableNumber(track.spotifyTotalTracks),
    ],
    [
      "total discs",
      normalizeComparableNumber(metadata.disc_total),
      normalizeComparableNumber(track.spotifyTotalDiscs),
    ],
  ];
  ...
  if (
    existingCoverHash &&
    requestedCoverHash &&
    existingCoverHash !== requestedCoverHash
  ) {
    differences.push("cover");
  }
  return differences;
}
async function findExistingVersionMatches(
  outputDir: string,
  ...
  for (const candidatePath of candidates) {
    const differences = await detectExistingTrackDifferences(
      candidatePath,
      track,
    );
    if (differences.length === 0) {
...
function analyzeBatchFilenameConflicts(
  ...
  const useAlbumSubfolder = Boolean(
    settings.folderTemplate?.includes("{album}") ||
    settings.folderTemplate?.includes("{album_artist}") ||
    settings.folderTemplate?.includes("{playlist}"),
  );
  const baseFilenameTemplate =
    settings.filenameTemplate || "{title} - {artist}";
  tracks.forEach((track, index) => {
    ...
    const trackNumberForTemplate =
      hasSubfolder && (track.track_number || 0) > 0
    ...
    if (
      ...
      targetDir = joinPath(
        ...
        targetDir,
        sanitizePath(folderName.replace(/\//g, " "), settings.operatingSystem),
      );
    }
    ...
    const expectedFilename = buildExpectedFilenameLikeBackend(
      track.name || "",
      displayArtist || "",
      track.album_name || "",
      displayAlbumArtist || "",
  ...
  const conflictGroups = Array.from(outputGroups.values()).filter(
    (group) => group.length > 1,
  );
  if (conflictGroups.length === 0) {
    return { conflictGroups: [], filenameOverrides: new Map() };
  }
  const previewLines = conflictGroups
    .slice(0, 3)
    .flatMap((group, groupIndex) => {
      const fileName =
        group[0].outputPath.split(/[/\\]/).pop() || group[0].outputPath;
      const lines = [`${groupIndex + 1}. ${fileName}`];
      for (const item of group.slice(0, 3)) {
        lines.push(
          `   - ${item.name} | ${item.artists} | ${item.album || "Sin album"}`,
        );
      }
      if (group.length > 3) {
  ...
  if (shouldPreserveAll) {
    for (const group of conflictGroups) {
      for (const item of group) {
        filenameOverrides.set(
          item.index,
          buildBatchDuplicateFilenameTemplate(
            baseFilenameTemplate,
    ...
    toast.info(
      "Se descargaran todas las versiones con nombres internos unicos y metadata original.",
    );
  } else {
    toast.info(
      "Se mantendra el comportamiento actual para canciones que generan el mismo archivo.",
    );
  }
...
export function useDownload(region: string) {
  ...
  const singleTrackQueueRef = useRef<
    Array<
      [
        string,
        string,
  ...
  const updateUiQueueItem = (
    queueId: string,
    patch: Partial<UiDownloadQueueItem>,
  ) => {
    setDownloadQueueItems((previous) =>
      previous.map((item) =>
  ...
  const downloadWithAutoFallback = async (
    ...
    if (trackName && artistName) {
      try {
        ...
        if (existenceResults.length > 0 && existenceResults[0].exists) {
          ...
          const { exactMatchPath, detectedDifferences } =
            await findExistingVersionMatches(
              outputDir,
              expectedFilename,
              versionReference,
            );
          ...
          filenameTemplateForRequest =
            buildSingleTrackDuplicateFilenameTemplate(
              baseFilenameTemplate,
              spotifyId,
              id,
            );
          toast.info(
            "Se descargara esta version con un nombre interno unico, manteniendo el titulo original en la app.",
          );
        }
      } catch (err) {
    ...
    if (service === "auto") {
      ...
      if (itemID) {
        const { MarkDownloadItemFailed } =
          await import("../../wailsjs/go/main/App");
        const finalError =
          fallbackErrors.length > 0
            ? fallbackErrors.join(" | ")
            : lastResponse.error || "All services failed";
    ...
    if (!singleServiceResponse.success && itemID) {
      const { MarkDownloadItemFailed } =
        await import("../../wailsjs/go/main/App");
      await MarkDownloadItemFailed(
        itemID,
        singleServiceResponse.error || "Download failed",
      );
    }
    ...
    try {
      while (singleTrackQueueRef.current.length > 0) {
        const [
          ...
          copyright,
          publisher,
        ] = singleTrackQueueRef.current.shift()!;
        ...
        try {
          ...
          if (response.success) {
            ...
            updateUiQueueItem(queueId, {
              status: "failed",
              error_message: response.error || "Download failed",
            });
          }
        } catch (err) {
  ...
  const handleDownloadTrack = async (
    ...
    singleTrackQueueRef.current.push([
      queueId,
      id,
      trackName,
      artistName,
  ...
  const handleDownloadSelected = async (
    ...
    const conflictAnalysis = analyzeBatchFilenameConflicts(
    ...
    const existenceChecks = selectedTrackObjects.map((track, index) => {
      ...
      return {
        ...
        use_album_track_number: useAlbumTrackNumber,
        filename_format:
          conflictAnalysis.filenameOverrides.get(index) ||
    ...
    for (let i = 0; i < tracksToDownload.length; i++) {
      ...
      try {
        ...
        const trackSettings = conflictAnalysis.filenameOverrides.has(
          originalIndex,
        )
          ? {
              ...settings,
              filenameTemplate:
                conflictAnalysis.filenameOverrides.get(originalIndex),
            }
          : settings;
        ...
        if (response.success) {
          if (response.already_exists) {
            skippedCount++;
            logger.info(
              `skipped: ${track.name} - ${displayArtist} (already exists)`,
            );
            setSkippedTracks((prev) => new Set(prev).add(id));
          } else {
            successCount++;
    ...
    if (folderName) {
      const playlistTrackPaths = selectedTrackObjects
        .map((track) => finalFilePaths.get(track.spotify_id || "") || "")
        .filter((path) => path !== "");
      if (playlistTrackPaths.length > 0) {
        try {
          logger.info(
            `syncing downloaded collection to host playlist: ${folderName}`,
          );
          const playlistPositions = selectedTrackObjects
            .filter((track) => finalFilePaths.has(track.spotify_id || ""))
            .map((track) => track.track_number // Use track_number as position
            );
          ...
          toast.warning(
            "La descarga termino, pero no se pudo sincronizar la playlist con tu biblioteca local.",
          );
        }
      }
    }
    logger.info(
      `batch complete: ${successCount} downloaded, ${skippedCount} skipped, ${errorCount} failed`,
    );
  ...
  const handleDownloadAll = async (
    ...
    for (let i = 0; i < tracksToDownload.length; i++) {
      ...
      const track = tracksToDownload[i];
      const originalIndex = tracksWithId.findIndex(
    ...
    setBulkDownloadType(null);
    shouldStopDownloadRef.current = false;
    const { CancelAllQueuedItems: CancelQueued } =
      await import("../../wailsjs/go/main/App");
    await CancelQueued();
    if (settings.createM3u8File && folderName) {
      try {
    ...
    if (folderName) {
      ...
      if (playlistTrackPaths.length > 0) {
        try {
          ...
          const playlistPositions = tracksWithId
            .filter((_, idx) => finalFilePaths[idx] !== "")
            .map((track, idx) => (track.track_number || idx + 1));
